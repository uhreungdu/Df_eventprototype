/* 자동 생성 파일입니다. 원본은 core/, app/, ui/app.js, data/에 있습니다.
 * 갱신: node scripts/build-standalone.mjs
 */
(() => {
"use strict";
const catalystStrategies = Object.freeze({
  attributeMultiplierAdd: ({ catalyst, attributeMultipliers }) => ({
  attributeMultipliers: {
    ...attributeMultipliers,
    [catalyst.Attribute]: (attributeMultipliers[catalyst.Attribute] ?? 0) + catalyst.EffectValue
  },
  gradeValueAdds: {}
  }),
  gradeValueAdd: ({ catalyst }) => ({
    attributeMultipliers: {},
    gradeValueAdds: { [catalyst.Attribute]: catalyst.EffectValue }
  })
});

function applyCatalystEffects(catalysts, baseAttributeMultipliers) {
  return catalysts.reduce((effects, catalyst) => {
    const strategy = catalystStrategies[catalyst.EffectType];
    if (!strategy) return effects;
    const next = strategy({ catalyst, attributeMultipliers: effects.attributeMultipliers });
    return {
      attributeMultipliers: { ...effects.attributeMultipliers, ...next.attributeMultipliers },
      gradeValueAdds: Object.entries(next.gradeValueAdds).reduce(
        (values, [attribute, value]) => ({ ...values, [attribute]: (values[attribute] ?? 0) + value }),
        effects.gradeValueAdds
      )
    };
  }, { attributeMultipliers: { ...baseAttributeMultipliers }, gradeValueAdds: {} });
}


function calculateQuality(materials, catalysts, multiplierTable) {
  if (materials.length === 0) return 0;

  const grouped = materials.reduce((groups, material) => ({
    ...groups,
    [material.Attribute]: [...(groups[material.Attribute] ?? []), material]
  }), {});
  const baseMultipliers = Object.fromEntries(
    Object.entries(grouped).map(([attribute, items]) => [attribute, multiplierTable.attributeMultipliers[items.length]])
  );
  const effects = applyCatalystEffects(catalysts, baseMultipliers);
  const subtotal = Object.entries(grouped).reduce((sum, [attribute, items]) => {
    const gradeAdd = effects.gradeValueAdds[attribute] ?? 0;
    const gradeSum = items.reduce((value, item) => value + item.QualityValue + gradeAdd, 0);
    return sum + gradeSum * effects.attributeMultipliers[attribute];
  }, 0);
  return subtotal * multiplierTable.diversityMultipliers[Object.keys(grouped).length];
}

function ceilQuality(quality) {
  return Math.ceil(quality);
}

function toggleSelection(selectedIds, materialId, maxSelection) {
  if (selectedIds.includes(materialId)) return selectedIds.filter((id) => id !== materialId);
...
function replenishHand(hand, deck, targetSize) {
  const needed = Math.max(0, targetSize - hand.length);
  const draw = drawHand(deck, needed);
  return { hand: [...hand, ...draw.hand], deck: draw.deck };
}

function processSelected(hand, selectedIds) {
  return {
    kept: hand.filter((material) => !selectedIds.includes(material.instanceId)),
    processed: hand.filter((material) => selectedIds.includes(material.instanceId))
  };
}

function applyCraftToOrder(progress, quality) {
  const next = {
    ...progress,
    accumulatedQuality: progress.accumulatedQuality + quality,
    craftsRemaining: Math.max(0, progress.craftsRemaining - 1)
  };
  return { ...next, status: evaluateOrder(next) };
}

function applyDiscardToOrder(progress) {
  return { ...progress, discardsRemaining: Math.max(0, progress.discardsRemaining - 1) };
}

function evaluateOrder(progress) {
  if (progress.accumulatedQuality >= progress.targetQuality) return "success";
  if (progress.craftsRemaining === 0) return "failed";
  return "playing";
}

function calculateTargetQuality(type, unit, balance) {
  const config = balance[type];
  if (!config || unit < 1) throw new RangeError("유효한 의뢰 타입과 1 이상의 일수 또는 단계가 필요합니다.");
  const qualityPerUnit = type === "daily" ? config.QualityPerDay : config.QualityPerStep;
  return config.BaseQuality + unit * qualityPerUnit;
}

function getDailyCycleKey(timestamp, resetHour) {
...
function getNextDailyResetTimestamp(timestamp, resetHour) {
...
function applyDailyCycle(progress, currentCycleKey) {
  if (progress.dailyCycleKey === currentCycleKey) {
    return { progress, didReset: false, interruptedDailyOrder: false };
  }

  const interruptedDailyOrder = progress.activeOrder?.type === "daily";
  return {
    progress: {
      ...progress,
      dailyCycleKey: currentCycleKey,
      dailyCompleted: false,
      activeOrder: interruptedDailyOrder ? null : progress.activeOrder
    },
    didReset: true,
    interruptedDailyOrder
  };
}

function createOrderSession({ type, step, seed, materialBox, equippedCatalystIds }) {
  return {
    type,
    step,
    seed,
    materialBox: { ...materialBox },
    equippedCatalystIds: [...equippedCatalystIds]
  };
}

function advancePrototypeDay(progress) {
  const interruptedDailyOrder = progress.activeOrder?.type === "daily" || progress.order?.type === "daily";
  return {
    progress: {
      ...progress,
      dailyDay: (progress.dailyDay ?? 1) + 1,
      dailyCompleted: false,
      activeOrder: progress.activeOrder?.type === "daily" ? null : progress.activeOrder
    },
    interruptedDailyOrder
  };
}

function createInitialInventory(materials, initialLoadout) {
  const materialBoxByGrade = initialLoadout.MaterialBoxByGrade ?? {};
  const warehouseByGrade = initialLoadout.WarehouseByGrade ?? {};

  return materials.reduce((inventory, material) => {
    const boxCount = materialBoxByGrade[material.Grade] ?? 0;
    const warehouseCount = warehouseByGrade[material.Grade] ?? 0;
    return {
      storage: { ...inventory.storage, [material.ID]: boxCount + warehouseCount },
      materialBox: { ...inventory.materialBox, [material.ID]: boxCount }
    };
  }, { storage: {}, materialBox: {} });
}
...
const data = {"materials":[{"ID":"fire-common","Name":"불씨 가루","Grade":"common","Attribute":"fire","QualityValue":1,"IconId":"fire"},{"ID":"fire-uncommon","Name":"타오르는 꽃잎","Grade":"uncommon","Attribute":"fire","QualityValue":2,"IconId":"fire"},{"ID":"fire-rare","Name":"홍염 결정","Grade":"rare","Attribute":"fire","QualityValue":3,"IconId":"fire"},{"ID":"fire-unique","Name":"불사조의 깃","Grade":"unique","Attribute":"fire","QualityValue":4,"IconId":"fire"},{"ID":"fire-legendary","Name":"태양의 심장","Grade":"legendary","Attribute":"fire","QualityValue":5,"IconId":"fire"},{"ID":"water-common","Name":"맑은 이슬","Grade":"common","Attribute":"water","QualityValue":1,"IconId":"water"},{"ID":"water-uncommon","Name":"푸른 산호","Grade":"uncommon","Attribute":"water","QualityValue":2,"IconId":"water"},{"ID":"water-rare","Name":"빙하 조각","Grade":"rare","Attribute":"water","QualityValue":3,"IconId":"water"},{"ID":"water-unique","Name":"해룡의 비늘","Grade":"unique","Attribute":"water","QualityValue":4,"IconId":"water"},{"ID":"water-legendary","Name":"심해의 눈물","Grade":"legendary","Attribute":"water","QualityValue":5,"IconId":"water"},{"ID":"light-common","Name":"반짝이는 모래","Grade":"common","Attribute":"light","QualityValue":1,"IconId":"light"},{"ID":"light-uncommon","Name":"빛무리 잎","Grade":"uncommon","Attribute":"light","QualityValue":2,"IconId":"light"},{"ID":"light-rare","Name":"여명 수정","Grade":"rare","Attribute":"light","QualityValue":3,"IconId":"light"},{"ID":"light-unique","Name":"천사의 깃털","Grade":"unique","Attribute":"light","QualityValue":4,"IconId":"light"},{"ID":"light-legendary","Name":"성광의 핵","Grade":"legendary","Attribute":"light","QualityValue":5,"IconId":"light"},{"ID":"dark-common","Name":"그림자 가루","Grade":"common","Attribute":"dark","QualityValue":1,"IconId":"dark"},{"ID":"dark-uncommon","Name":"밤안개 버섯","Grade":"uncommon","Attribute":"dark","QualityValue":2,"IconId":"dark"},{"ID":"dark-rare","Name":"월식 파편","Grade":"rare","Attribute":"dark","QualityValue":3,"IconId":"dark"},{"ID":"dark-unique","Name":"악마의 뿔","Grade":"unique","Attribute":"dark","QualityValue":4,"IconId":"dark"},{"ID":"dark-legendary","Name":"심연의 심장","Grade":"legendary","Attribute":"dark","QualityValue":5,"IconId":"dark"}],"catalysts":[{"ID":"fire-amplifier","Name":"홍염 증폭석","EffectType":"attributeMultiplierAdd","EffectValue":0.25,"Attribute":"fire","IconId":"fire"},{"ID":"water-amplifier","Name":"빙결 증폭석","EffectType":"attributeMultiplierAdd","EffectValue":0.25,"Attribute":"water","IconId":"water"},{"ID":"light-refiner","Name":"광휘 정제석","EffectType":"gradeValueAdd","EffectValue":1,"Attribute":"light","IconId":"light"},{"ID":"dark-refiner","Name":"심연 정제석","EffectType":"gradeValueAdd","EffectValue":1,"Attribute":"dark","IconId":"dark"}],"orders":[{"ID":"daily-1","Name":"오늘의 연금 의뢰","Type":"daily","Step":1,"TargetQuality":75,"CraftCount":5,"DiscardCount":3,"RewardId":"alchemy-coin-20","TextId":"daily_intro"},{"ID":"challenge-1","Name":"견습 연금술사의 시험","Type":"challenge","Step":1,"TargetQuality":85,"CraftCount":5,"DiscardCount":3,"RewardId":"record","TextId":"challenge_1"},{"ID":"challenge-2","Name":"숙련 연금술사의 시험","Type":"challenge","Step":2,"TargetQuality":100,"CraftCount":5,"DiscardCount":3,"RewardId":"record","TextId":"challenge_2"},{"ID":"challenge-3","Name":"대연금술사의 시험","Type":"challenge","Step":3,"TargetQuality":115,"CraftCount":5,"DiscardCount":3,"RewardId":"record","TextId":"challenge_3"}],"multipliers":{"attributeMultipliers":{"1":1,"2":1.5,"3":2,"4":2.5,"5":3.5},"diversityMultipliers":{"1":1,"2":1.25,"3":1.5,"4":1.75}},"shopItems":[{"ID":"material-pot","Name":"재료 항아리","ItemType":"materialPot","Price":150,"Currency":"magicCrystal","DropTable":{"Count":5,"common":0.3,"uncommon":0.3,"rare":0.25,"unique":0.1,"legendary":0.05}},{"ID":"catalyst-box","Name":"속성별 촉매 상자","ItemType":"catalystBox","Price":250,"Currency":"magicCrystal","DropTable":"selectOneUnowned"}],"orderBalance":{"daily":{"BaseQuality":30,"QualityPerDay":5},"challenge":{"BaseQuality":35,"QualityPerStep":5}},"initialLoadout":{"MaterialBoxByGrade":{"common":6,"uncommon":0,"rare":0,"unique":0,"legendary":0},"WarehouseByGrade":{"common":0,"uncommon":0,"rare":0,"unique":0,"legendary":0}}};
